...
use rdf_fusion_encoding::plain_term::{
    PlainTermArray, PlainTermArrayElementBuilder, PlainTermEncoding, PlainTermScalar,
};
...
impl ObjectIdMapping for MemoryObjectIdMapping {
        ...
        for term in terms {
            match term {
                Ok(term) => {
                    ...
                    let bytes: [u8; 4] = object_id
                        .as_object_id()
                        .as_bytes()
                        .try_into()
                        .expect("ObjectId must be 4 bytes");

                    result.append_value(u32::from_be_bytes(bytes));
                }
        ...
        for term in terms {
            match term {
                Some(EncodedTerm::NamedNode(value)) => {
                    builder.append_named_node(NamedNodeRef::new_unchecked(value.as_ref()));
                }
                Some(EncodedTerm::BlankNode(value)) => {
                    builder.append_blank_node(BlankNodeRef::new_unchecked(value.as_ref()));
                }
                ...
                Some(EncodedTerm::LangString(value, language)) => {
    ...
    fn decode_array_to_typed_value(
        ...
        for typed_value in typed_values {
            let typed_value = typed_value.as_ref().and_then(Option::<TypedValueRef>::from);
            match typed_value {
                None => builder.append_null()?,
