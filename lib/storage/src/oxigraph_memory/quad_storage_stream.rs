...
use futures::Stream;
use rdf_fusion_model::{AResult, BlankNodeMatchingMode, DFResult};
...
fn object_id_to_u32(value: &crate::oxigraph_memory::object_id::EncodedObjectId) -> u32 {
    ...
    fn encode_batch(
        ...
        if let Some((_, builder)) = &mut self.graph {
            for quad in quads.iter().take(count) {
                ...
                match &value.0 {
                    ...
                    Some(value) => builder.append_value(object_id_to_u32(value)),
                }
            }
        }
        ...
        if let Some((_, builder)) = &mut self.subject {
            for quad in quads.iter().take(count) {
                ...
                builder.append_value(object_id_to_u32(value))
            }
        }
    ...
    fn finish(self) -> AResult<RecordBatch> {
        fn try_add_column(
            ...
            if let Some((var, mut builder)) = column {
                fields.push(Field::new(var.name(), encoding.data_type().clone(), nullable));
